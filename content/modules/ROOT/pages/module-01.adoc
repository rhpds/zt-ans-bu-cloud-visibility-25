...
. **Configure the `Source Variables` section.**
+
...
+
Click the *Launc inventory update* button in the top right to retrieve the host inventory from AWS EC2.
...
+
First, click the *AWS Inventory* breadcrumb, in order to see the Hosts tab. Then, click the *Hosts* tab. You should now see two hosts listed, dynamically pulled from AWS.
+
